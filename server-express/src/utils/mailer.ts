  ...
  const mailOptions = {
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e5e7eb; border-radius: 8px;">
        <h2 style="color: #2563EB; text-align: center;">DevPilot Verification Code</h2>
        <p>Hi ${name},</p>
        <p>To complete your request and secure your account, please verify your email address using the One-Time Password (OTP) below:</p>
  ...
  if (awsHost && awsUser && awsPass && !awsUser.includes("your_aws_ses_user")) {
    try {
      const transporter = nodemailer.createTransport({
        ...
        port: awsPort,
        secure: awsPort === 465,
        auth: { user: awsUser, pass: awsPass },
      });
      
      const fromEmail = process.env.SENDER_EMAIL || "gtc.ashish1@gmail.com";
      
      const info = await transporter.sendMail({ ...mailOptions, from: `"DevPilot Support" <${fromEmail}>` });
      console.log(`[Mailer] OTP email sent successfully via AWS SES to ${email}. Message ID: ${info.messageId}`);
      return info;
    } catch (error) {
      console.error(`[Mailer] AWS SES failed to send email:`, error);
      throw error;
    }
  } else {
    console.error(`[Mailer] AWS SES is not properly configured in the environment variables.`);
  }
};
